namespace Raspoznavayka {
    namespace Math {
        ...
        //whole CInDBMelody
        std::uint16_t getLevenshtein( const CRecordedMelody&, 
									  const CInDBMelody& );
        ...
        std::uint16_t getLevenshteinFixed( const CRecordedMelody&, 
										   const CInDBMelody&, 
										   std::int64_t offset );
        ...
        std::uint16_t levenshtein( 
				const std::vector< Raspoznavayka::interval_t >&, 
				const std::vector< Raspoznavayka::interval_t >& ); 
    }
}
